...
def test_alconna_special_help():
    alc4 = Alconna(
        "Cal",
        help_text="计算器 Usage: Cal <expression>; Example: Cal -sum 1 2;",
        options=[
            Subcommand(
                "-div",
                options=[Option("--round|-r", Args.decimal[int], help_text="保留n位小数")],
                args=Args(num_a=int, num_b=int), help_text="除法计算"
            )
        ],
    )
    print('')
...
def test_fuzzy():
    alc15 = Alconna(main_args="foo:str", headers=['!core15'], is_fuzzy_match=True)
